...
**Caution**: turning on SPI0 requires turning off M55's UART1 console (it shares GPIO23/24). This lab's `lab/prj.conf` already sets `CONFIG_UART_CONSOLE=n` and `CONFIG_CONSOLE=n`. To view M55's console you must use the **direct USB-C connection (J14, 115200bps)** — the J25 header method cannot be used in this lab.
...
1. Open both M4's console (J24, 230400bps) and M55's console (J14 USB-C, 115200bps).
